...
class NewCardForm extends Component {
  ...
  removeTag(event) {
    ...
    const filteredTags = this.state.cardTags.filter(tag => tag.value !== id)
    this.setState({ errorType: null, error: '', cardTags: filteredTags })
  }
  ...
  renderTagSelector() {
    ...
    return (
      <div className={`mb-0 pt-3 form-group${errorType === errorTypes.tag ? ' has-warning' : ''}`}>
        ...
        <Select
          options={this.getFilteredTags(searchValue)}
          id="tagSelection"
          size="lg"
  ...
  renderPickCountSelector() {
    ...
    return (
      <div className="mb-0 pt-3 form-group">
        ...
        <div
          className="btn-group btn-group-lg sah-btn-group-justified"
          id="inputPickCount"
          role="group"
        >
          <button
